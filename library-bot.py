...
async def tag(interaction, taglist: str):
    ...
    if len(matches) == 0:
        ...
        try:
            ...
            await interaction.followup.send("Vel has too many audios tagged " + tagstring.lower() + "to display without exceeding the Discord character limit! You can limit results by adding another tag you enjoy, or find a random audio with the tag " + tagstring.lower() + "with the `/randomaudio` command by using the tag option!" )
...
async def stream(interaction):
    ...
    await interaction.response.defer()
    stream_info = 'Vel streams live every weekend on [Twitch](https://www.twitch.tv/velslibrary). The next streams will be ' + twitch_time + '!'
    stream_embed = discord.Embed(title = "Vel's Livestreams", description = stream_info, url = "https://www.twitch.tv/velslibrary")
