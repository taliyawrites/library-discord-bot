...
def get_dsp_audios(audios):
    dsp_audios = []
    for audio in audios: 
        if "darksideplayground" in audio.link().lower() or "Emilia Rose" in audio.writer():
...
def active_member(userID):
    try:
        ...
        patron_role = client.get_guild(GUILD).get_role(1154619473773465610)
        if patron_role in member.roles:
            return True
        else:
            return False
    except:
...
async def setup_hook():
    ...
    global random_seed, good_girl, pet_count, edge_counter, cum_permission_ids, daily_audio, snack_requests, birthdays, twitch_time, live_time, event_times, gull_counter
    ...
    if len(matches) == 0:
        ...
        if len(full_overlap_matches) == 0: 
            ...
            if respondQ:
                ...
                if optionTextQ:
                    link_string = link_string + f'*These are the closest matches found for your "{phrase}" search. Not the audio you were looking for? Redo the `/title` command search with the option "show_all" to see every audio that partially matches your query!*'
                try:
...
async def tag(interaction, taglist: str):
    ...
    if len(matches) == 0:
        if "fourth wall break" in taglist.lower():
...
async def rules(interaction):
    ...
    rules_string = "Rules, etiquette, and info for live audio recordings! \n- He's recording porn, and while it's okay if you'd like to \"play along\"/\"roleplay\"/\"follow along\" with the audio, please don't tell us what you're doing when you're playing. \n- Please be conscientious of your behavior to make things easier on our mods — the live chat moves very fast which makes it difficult to moderate, so everyone's cooperation with the server rules and the additional rules/etiquette outlined here is greatly appreciated!  \n- If you see someone else violating these rules or any broader server rules during the live, please submit a ticket using the https://discord.com/channels/1148449914188218399/1192558831222411294 with a link to the chat message(s) in question so we can handle it! \n- After the live, all chatter, memes, and spoilers about unreleased audios (so anything recorded during live that hasn't been posted to Patreon/Reddit/Quinn yet) is restricted to the https://discord.com/channels/1148449914188218399/1274877049118396416 channel to avoid spoiling the audios for folks who didn't attend the live recording! \n- Every audio Vel records during these lives is improv unless otherwise noted! \nAll server rules (which you can read/review at any time in https://discord.com/channels/1148449914188218399/1374549206286733322) of course still apply during lives in addition to the extra guidelines laid out here. For more information about the server rules, see the resources available in the <id:guide>!"
    await interaction.followup.send(rules_string)
...
async def cum(interaction):
    await interaction.response.defer()
    global edge_counter
    mod_ids = [1169014359842885726, 1089053035377999912, 1082532069420314687,1262940885251784785, 1307772265517023454, 619414762572349440, 98246610651201536]
...
async def gull(interaction):
    ...
    if gull_counter == 249: 
        await interaction.channel.send("There is approximately 13.5meters of gull beak in Vel’s abode. That’s about the length of an adult grey whale or city bus. That’s a lotta squawking. (Gull facts by Mollie!)")
    if gull_counter == 300: 
        await interaction.channel.send("If all the gulls in Vel’s home spread their wings and stood in a line, it would stretch for a quarter mile. (Gull facts by Mollie!)")
    if gull_counter == 331: 
        await interaction.channel.send("If all the gulls in Vel’s abode molted at once, they would produce a pile of just over *2 million* feathers. I hope he can still bust under these conditions. (Gull facts by Mollie!)")
    if gull_counter == 386: 
        await interaction.channel.send("The horde of gulls needs to be appeased with almost 9,000 flatfish daily. Consider getting something from his throne because the man is supporting a whole flock of our retaliatory violence! (Gull facts by Mollie!)")
    if gull_counter == 400: 
    ...
    if before.roles != after.roles:
        ...
        if patron in after.roles and patron not in before.roles:
            if timeout not in before.roles:
                ...
                print(f"Patreon role manually revoked for user {before.name}.")
...
async def on_error(interaction, error):
    ...
    if isinstance(interaction.channel, discord.DMChannel):
        await taliya.send("**ERROR:** in *" + error.command.name + "* command in DM with " + interaction.user.display_name + " (" + trce +  options_string + str(error) + response_string)
    else:
        await taliya.send("**ERROR:** in *" + error.command.name + "* command in " + interaction.channel.jump_url + " (" + trce + options_string + str(error) + response_string)
